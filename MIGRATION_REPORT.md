...
15. **CSP made compatible with Astro's script inlining.** Astro inlines small
    ...
    because the preview server was not applying the headers. Fixed by letting
    Astro own the policy so it emits hashes for what it inlined, moving the
    five affected scripts into `src/scripts/` modules, and moving the eight
    remaining inline `style` attributes into scoped CSS. `npm run preview` now
    serves the production headers, and the QA suite fails if an inline script
    is ever left unhashed.
...
- **Content-Security-Policy is generated by Astro** (`security.csp`) as a
  `<meta>` tag carrying SHA-256 hashes for the script chunks Astro inlines.
  `script-src` is `'self'` plus those hashes — no `'unsafe-inline'`.
  `style-src` is `'self'` only: the eight inline `style` attributes inherited
  from the Elementor markup were moved into scoped CSS so no exception is
  needed. `vercel.json` supplies `frame-ancestors 'none'` (a `<meta>` CSP
  cannot express it) plus `X-Content-Type-Options`, `Referrer-Policy`,
  `Permissions-Policy`, `X-Frame-Options: DENY`, HSTS and
...
`npm run preview` applies the production security headers, so the suites below
exercise the site under the same CSP it will run with on Vercel.
...
slugs, valid product/category relationships, no orphan product pages, that
every inline script chunk is covered by a CSP hash, that `script-src` never
falls back to `'unsafe-inline'`, and that no inline `style` attributes remain.
