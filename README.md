...
| `npm run preview` | Serves the production build with Vercel's routing rules (trailing slashes, 301s, 404, 410) **and the production security headers** |
...
- The CSP is generated by Astro (`security.csp` in `astro.config.mjs`) as a
  `<meta>` tag with SHA-256 hashes for the script chunks Astro inlines.
  **Client-side code must live in a `src/scripts/` module and be imported by
  the component** — that keeps it hashable and keeps `script-src` free of
  `'unsafe-inline'`. Avoid inline `style` attributes for the same reason.
